...
var PRD = (function IIFE( logprefix, verbosemsg ) {
   ...
   * @since 1.0.7
   *
   * @param  {Array.<string[]>} arr         ECMA Array of arrays.
   * @param  {boolean}          ignorecase  Changes string comparison to case insensitive.
   *                                        This also causes the casing of the results will match the casing of their
   *                                        first time appearing in the list provided.
   *
  ...
  function intersectArrays( arr, ignoreBool ) {
    ...
    try {
      if ( arr.length === 0 ) {
        logerror( fname + 'Must have array elements to compare' );
        return [];
      }
      if ( arr.length === 1 ) {
        return arr[ 0 ];
        ...
        arr.forEach(function ( key, ind ) {
          var next = ind + 1;
          if ( next < arr.length ) {
            result = compare( result[ 2 ], arr[ next ], ignoreBool );
          }
        });
        return result[ 2 ];
      }
    } catch ( e ) {
      logerror( fname + 'Must submit array as parameter. Error: ' + e.message );
      return [];
    }
  }
